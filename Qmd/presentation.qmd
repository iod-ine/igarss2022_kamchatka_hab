...
You can see the text and the code to generate a map in the same document. <NEXT>
...
This presentation is rendered to HTML and can be opened in any modern browser. <NEXT>
This removes the need to manually keep track of any computational output in the resulting documents.
If code changes, everything depending on it is updated right away. <NEXT>
...
::: {.notes}
The components required to recreate our results or to create new results like ours are all open source and free. <NEXT>
We used a combination of Python and R scripts to automate the processing and generate all maps.
You don't have to use both at the same time, you can use one language you are more comfortable with. <NEXT>
To process the Sentinel imagery, we used SNAP.
More specifically, we added a Python wrapper around SNAP graph processing tool to run the computations on a headless workstation using a simple Python script. <NEXT>
And to generate the output, we used R Markdown and its more recent successor, Quarto.
These are literate programming frameworks. <NEXT>
To version control and openly host the code, we used Git and GitHub.
It's even possible to host the web versions of the papers and presentations directly from GitHub for free. <NEXT>
...
::: {.notes}
If you were interested in recreating our results, everything comes down to three steps. <NEXT>
The first is to actually run the processing pipeline.
This will execute the graph from the previous slide and start tracking which tasks have been updated and need to be rerun in case you decide to change something in the code. <NEXT>
